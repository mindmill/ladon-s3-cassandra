...
class DebugPageController : FrameController() {
    ...
    fun searchId(model: MutableMap<String, Any>, callContext: LadonCallContext,
        ...
        if (searchpath != null) {
            val objs = dataDAO.listAllMetadata(callContext, repoid, searchpath, "", 1000, true).first
        ...
        if (!time.isNullOrEmpty()) {
            val result = tokenDao.getAllChangesSince(callContext, repoid, UUIDs.startOf(datetime.time).toString(), BigInteger.valueOf(1000)).toTableObjectList()
    ...
    fun download(response: HttpServletResponse, @RequestParam key: String, callContext: LadonCallContext) {
        ...
        val meta = dataDAO.getMetadata(callContext, resource) ?: throw LadonObjectNotFoundException(key.toString())
    ...
    fun List<Props>.toTableObject(): TableObject {
        ...
        return TableObject("Properties", headers, mapIndexed { i, e ->
            TableRow(listOf(
                    TableCell(e.id),
                    TableCell(e.type),
                    TableCell(e.value)),
    ...
    fun Acl.toTableObject(): TableObject {
        ...
        return TableObject("Acl", headers, content.mapIndexed { i, ace ->
            TableRow(listOf(
